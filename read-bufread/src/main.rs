...
fn main() {
    ...
    // Create a buffered reader for the file to improve reading efficiency
    // BufReader reduces the number of I/O operations by reading chunks of data
    let mut reader: BufReader<File> = BufReader::new(file);

    // Size of each block to be read from the file. 2MB in this case
    const BLOCK_SIZE: usize = 2_097_152;

    // Empty vector or u8 (bytes) with a length of 2MB, filled with zeros (0)
